/*
 * scene_instanced_colored.frag.hlsl — Blinn-Phong lighting with per-instance color
 *
 * Part of forge_scene.h — same as scene.frag.hlsl but uses the per-instance
 * color interpolated from the vertex shader, multiplied by the uniform
 * base_color (allows a global tint on top of per-instance colors).
 *
