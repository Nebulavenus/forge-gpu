...
- [ ] **Audio Lesson 02 — Sound Effects** — Fire-and-forget one-shot playback with automatic cleanup; polyphony (multiple overlapping instances of the same sound); fade-in and fade-out with configurable duration; `forge_audio_play_oneshot()` and `forge_audio_fade_out()` API additions to `forge_audio.h`; active-source pool with fixed capacity and automatic slot reclamation; UI panel showing active source list with per-source volume bars and fade state; demo scene with 3D objects that trigger short effects (footstep, impact, click) on key press and a looping ambient sound with fade controls; audio files: 3–4 short one-shots (< 1s), 1–2 medium one-shots (1–3s), 1 looping ambient — user-supplied WAVs via `audio.conf` (Sound Universe library or freesound.org)
