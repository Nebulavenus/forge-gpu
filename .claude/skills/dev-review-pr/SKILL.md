...
- **`--local` flag**: transition to local-only review after fetching remaining
  GitHub feedback. When set, the skill fetches all outstanding PR feedback,
  addresses it, then uses `dev-local-review` (the CodeRabbit CLI) instead of
  pushing and requesting GitHub re-reviews. This avoids burning GitHub review
  rounds on iterative fixes.

## `--local` mode

When the `--local` flag is provided, the skill transitions to local-only
review after fetching the initial GitHub feedback. This avoids burning
GitHub review rounds on iterative fixes.

**How it works:**

1. Steps 1-4 run normally — fetch GitHub feedback, classify themes, build
   verification plans, execute fixes.
2. Instead of step 5 (push + request GitHub re-review), run the CodeRabbit
   CLI locally to validate the fixes.
3. If the local review finds new issues, fix them and re-run locally. Repeat
   until the local review is clean.
4. Once clean, push all commits in a single push, request `@coderabbitai
   resolve` on the PR, and proceed to merge.

**When `--local` is set, replace steps 5-6 with step 5L below.** Steps 1-4
and step 7 are unchanged.

### 5L. Local review loop (replaces steps 5-6 when `--local` is set)

After ALL themes are resolved and build/test/lint checks pass locally
(step 4d), do NOT push yet. Instead:

#### 5L.1. Run the local CodeRabbit review

```text
/coderabbit:review --base main -c .coderabbit.yaml
```

If the Claude Code plugin is not available, fall back to the CLI:

```bash
coderabbit review --base main -c .coderabbit.yaml
```

#### 5L.2. Evaluate results

**If the local review returns no findings:**

- The fixes are clean. Proceed to 5L.4 (push and resolve).

**If the local review returns new findings:**

- Present the findings to the user, grouped by theme (same as step 3).
- Run impact analysis and build verification plans (same as step 4).
- Execute fixes (same as step 4c).
- Re-run build/test/lint checks (same as step 4d).
- Go back to 5L.1 and re-run the local review.

**Cap at 3 local iterations.** If the local review still has findings
after 3 rounds, stop and report to the user — the issues may require a
design-level fix (see step 5.5 escalation).

#### 5L.3. Commit locally (do NOT push yet)

After each fix round, commit locally so changes are reviewable:
...
These are local commits only — no push until the local review is clean.

#### 5L.4. Push once and resolve

Once the local review is clean:

1. **Push all commits in a single push:**

   ```bash
   git push
   ```

2. **Request CodeRabbit resolve** (not a new review — the local review
   already confirmed the code is clean):

   ```bash
   gh pr comment <pr-number> --body "@coderabbitai resolve"
   ```

3. **Do NOT request `@coderabbitai review`** — that would trigger a new
   GitHub review round, defeating the purpose of `--local`. The local
   CLI already ran the same analysis.

4. **Wait for CI checks to pass**, then proceed to step 7 (merge).

   If CI fails on something the local review didn't catch (e.g. a
   platform-specific build issue), fix it locally, re-run the local
   review, and push again.

#### 5L.5. Handle the resolve-then-merge sequence

The same catch-22 from step 6a applies: if `main` has moved ahead, merging
main dismisses the approval from `@coderabbitai resolve`.

- If the branch is up to date with main: merge immediately after CI passes.
- If the branch is behind main: merge main, push, wait for CI, then use
  `--admin` to merge (with user confirmation, same as step 6a.d).

**After 5L.4 completes, skip steps 5 and 6 entirely and go to step 7.**
